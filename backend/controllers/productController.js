...
const categories = [
  ...
  },
  {
    ...
    genders: ["Men", "Women", "Unisex"],
    images: [
      "https://images.unsplash.com/photo-1517836357463-d25ddfcbf042?auto=format&fit=crop&w=900&q=80",
      "https://images.unsplash.com/photo-1461896836934-ffe607ba8211?auto=format&fit=crop&w=900&q=80",
      "https://images.unsplash.com/photo-1572802419224-7b1aca0c5ed0?auto=format&fit=crop&w=900&q=80",
    ],
    description:
      "High-performance sports equipment and apparel for athletes of all levels.",
  },
];
...
const generateProducts = () => {
  const products = categories.flatMap((category) =>
    Array.from({ length: 25 }, (_, index) => createProduct(category, index)),
  );
  return products;
};
...
const seedSampleProducts = async () => {
  ...
  // Always reseed if not exactly 250 products
  if (count !== 250) {
    console.log(`Clearing ${count} old products and reseeding...`);
    await Product.deleteMany({});
    const products = generateProducts();
    await Product.insertMany(products);
    console.log("✓ Seeded " + products.length + " products successfully.");
  } else {
    console.log("✓ Database already has 250 products.");
  }
};
...
const seedProducts = async (req, res) => {
  ...
  const createdProducts = await Product.insertMany(generateProducts());
