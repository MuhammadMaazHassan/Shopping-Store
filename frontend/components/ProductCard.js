...
// ------------------------------------------------------------
// StarRating – renders full, half, and empty stars
// ------------------------------------------------------------
...
// ------------------------------------------------------------
// AddToCartButton – handles loading, disabled states and styling
// ------------------------------------------------------------
...
const ProductCard = ({ product, onAddToCart }) => {
  ...
  const [isWishlisted, setIsWishlisted] = useState(false); // optional, you can remove

  // Stock handling – default to 1 if countInStock is missing (so product is considered in stock)
  ...
  return (
    <article className="group relative flex h-full flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm transition-all duration-300 hover:-translate-y-1.5 hover:shadow-xl dark:border-slate-800 dark:bg-slate-950">
      ...
      {/* Wishlist heart button (optional – remove if not needed) */}
      <button
        onClick={(e) => {
          e.preventDefault();
      ...
      >
        ...
        {/* Badges overlay (New, Sale %, Out of stock) */}
      ...
      <div className="flex flex-1 flex-col p-5">
        ...
        {/* Low stock warning (only if in stock but less than 5) */}
        {!outOfStock &&
        ...
        {/* Delivery options – if the product object includes a `delivery` object */}
        {product.delivery && (
...
// PropTypes for runtime validation – matches your existing product structure
