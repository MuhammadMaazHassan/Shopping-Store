...
export default function CartPage() {
  ...
  const { cartItems, removeFromCart, updateQuantity, clearCart } = cartContext as any;
    ...
    return (
      <div className="mx-auto max-w-4xl text-center py-20 animate-fadeInUp">
        <div className="mx-auto h-64 w-64 mb-8">
          <img src="/images/checkout_success.png" alt="Success Illustration" className="w-full h-full object-contain drop-shadow-2xl hover:scale-105 transition-transform duration-500" />
        </div>
        <h1 className="text-5xl font-bold text-slate-900 tracking-tight">Order Confirmed!</h1>
        <p className="mt-6 text-xl text-slate-600 max-w-2xl mx-auto leading-relaxed">
          Thank you, <span className="font-semibold text-slate-900">{address.fullName}</span>! Your items are being prepared. They will be shipped to <span className="font-semibold text-slate-900">{address.street}, {address.city}</span> via {deliveryMethod} delivery.
        </p>
        <Link href="/" className="mt-10 inline-block rounded-full bg-brand-600 px-8 py-4 font-semibold text-white shadow-lg shadow-brand-500/30 hover:bg-brand-700 hover:-translate-y-1 transition duration-300">
          Continue Shopping
        </Link>
      </div>
    );
      ...
      <div className="mx-auto max-w-7xl space-y-8 animate-fadeInUp px-4 sm:px-6 lg:px-8">
        ...
        <div className="overflow-hidden rounded-[2rem] bg-slate-900 p-10 sm:p-14 text-white shadow-lg relative min-h-[320px] flex flex-col justify-center group">
          ...
          <div className="relative z-10 max-w-2xl">
            <span className="inline-block px-4 py-1.5 bg-white/20 backdrop-blur-md border border-white/30 rounded-full text-xs font-bold uppercase tracking-widest mb-4 shadow-sm text-white">Secure Checkout</span>
            <h1 className="text-4xl sm:text-5xl font-bold tracking-tight text-white drop-shadow-lg">Complete your purchase</h1>
            <p className="mt-4 text-slate-200 text-lg leading-relaxed drop-shadow-md">
              Review your items, choose a delivery method, and tell us where to send your order. Your items are reserved for 15 minutes.
            </p>
          </div>
        </div>
        ...
        {cartItems.length === 0 ? (
          <div className="rounded-[2rem] border border-slate-200 bg-slate-50 p-16 text-center shadow-sm">
            ...
            <p className="mt-3 text-slate-600 text-lg">Find your next favorite product in the store.</p>
            <Link
              href="/"
          ...
          <div className="grid gap-10 lg:grid-cols-[1.5fr_1fr] items-start">
            ...
            <div className="space-y-8">
              ...
              <div className="rounded-[2rem] border border-slate-200 bg-white p-8 shadow-sm">
                ...
                <div className="space-y-6">
                  {cartItems.map((item: any) => (
                    <div key={item._id} className="rounded-[1.5rem] border border-slate-100 bg-slate-50 p-4 flex flex-col sm:flex-row items-center gap-6 relative">
                      <img src={item.image} alt={item.name} className="h-28 w-28 rounded-2xl object-cover shadow-sm" />
                      <div className="flex-1 w-full text-center sm:text-left">
                        <h3 className="text-lg font-bold text-slate-900">{item.name}</h3>
                        <p className="text-sm text-slate-500 font-medium tracking-wide uppercase mt-1">{item.brand}</p>
                        <p className="mt-2 font-semibold text-brand-700">${item.price.toFixed(2)}</p>
                      </div>
                      <div className="flex items-center gap-3 bg-white p-2 rounded-full border border-slate-200 shadow-sm">
                        <button onClick={() => updateQuantity(item._id, Math.max(1, item.quantity - 1))} className="w-8 h-8 flex items-center justify-center rounded-full hover:bg-slate-100 text-slate-600 font-bold">-</button>
                        <span className="w-4 text-center font-semibold text-slate-900">{item.quantity}</span>
                        <button onClick={() => updateQuantity(item._id, item.quantity + 1)} className="w-8 h-8 flex items-center justify-center rounded-full hover:bg-slate-100 text-slate-600 font-bold">+</button>
                      </div>
                      <button onClick={() => removeFromCart(item._id)} className="absolute top-4 right-4 text-slate-400 hover:text-red-500 transition">
              ...
              <div className="rounded-[2rem] border border-slate-200 bg-white p-8 shadow-sm">
                ...
                <div className="grid sm:grid-cols-3 gap-4">
                  {[
                    { id: "standard", title: "Standard", time: "4-7 Days", price: 0 },
                    { id: "express", title: "Express", time: "2-3 Days", price: 9.99 },
                    { id: "overnight", title: "Overnight", time: "Next Day", price: 24.99 },
                  ].map((method) => (
                    <label key={method.id} className={`cursor-pointer rounded-[1.5rem] border-2 p-5 transition-all ${deliveryMethod === method.id ? 'border-brand-600 bg-brand-50 shadow-md' : 'border-slate-100 bg-white hover:border-brand-200'}`}>
                      <input 
                        type="radio" 
                        name="deliveryMethod" 
                        className="sr-only" 
                      ...
                      <div className="flex flex-col h-full justify-between">
                        <div>
                           ...
                           <p className="text-sm text-slate-500 mt-1">{method.time}</p>
                        </div>
                        <p className="font-bold text-brand-700 mt-4">{method.price === 0 ? "FREE" : `$${method.price}`}</p>
                      </div>
                    </label>
                  ))}
                </div>
            ...
            <aside className="sticky top-8 space-y-6 rounded-[2rem] border border-slate-200 bg-white p-8 shadow-xl shadow-slate-200/40">
              <h2 className="text-2xl font-semibold text-slate-900 mb-2">Summary</h2>
              <div className="space-y-4 text-slate-600">
                ...
                <div className="flex items-center justify-between pb-4 border-b border-slate-100">
                  ...
                  <span className="font-semibold text-slate-900">{deliveryCost === 0 ? "Free" : `$${deliveryCost.toFixed(2)}`}</span>
                </div>
